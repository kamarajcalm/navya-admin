...
import { IonMenuButton, IonButtons, IonTitle, IonApp, IonRouterOutlet, IonAlert, IonHeader, IonContent, IonToolbar } from '@ionic/react';
...
import {ahLogoWhite} from './assets/images/index';
...
class ExpertApp extends React.PureComponent {
  ...
  appBackButtonCb = state => {
     let initPage = isLoggedIn() ? '/user/dashboard' : '/login';
  ...
  render() {
      ...
      globalVars.setNApiHost(getConfig().urlRootNAPI);
      globalVars.setCBApiHost(getConfig().urlRootCB);
        ...
        <IonApp className={`${referrer} ${isMobileApp() ? 'mobile-app' : 'web-app'}`}>
            ...
            <IonHeader>
              <IonToolbar className='main-header'>
                <IonButtons slot="start"><IonMenuButton style={{color: "#fff"}}/></IonButtons>
                <IonButtons>
                  <img alt="ah-logo" height="48" className='p15' src={ahLogoWhite} /> 
                </IonButtons>
              </IonToolbar>
            </IonHeader>
            <IonContent>
              ...
              buttons={[
                {
                  ...
                  handler: () => {
                    ...
                    if (!!localStorage.getItem('zombiedCaseFlag')) {
                      ...
                      window.location = window.location.origin + '/user/dashboard';
                    }
                  }
                },
                {
