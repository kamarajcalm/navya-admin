...
class tokenManager {
  ...
  setPermissions(permissions=[]) {
    if(permissions.length) {
      localCache.set('user_permissions', permissions);
    }
  }
  ...
  addHeaders(req) {
    ...
    let { token=null, userid=null } = getData();
  ...
  clearToken() {
    ...
    localCache.remove('user_permissions');
  }
  ...
  clientIdAuth(options, req) {
    req.set('client-id', "2KzyxDKLjGyWirc128asEoQbPd7xMOaX96VxMIBl")
    req.set('authorization', window.localStorage.getItem('token'))
    req.set('userid', window.localStorage.getItem('userid'));
  }  
}
