...
class PendingInvites extends React.PureComponent {
    ...
    customDateFilter = async (startDate, endDate) => {
        await this.getPendingInvites(formatDate(startDate), formatDate(endDate));
    };
    ...
    getPendingInvites = async (startDate, endDate) => {
        ...
        const response = await api.getPendingInvites(payload);
        if (response && response.success && response.data && response.data.result) {
            ...
            const newColumns = response.data.columns.map(v => {
                if (v.field === 'date_assigned' || v.field === 'due_date') {
    ...
    viewClick = (row) => {
        this.props.history.push("/user/dashboard/awaiting-acceptance/" + row.survey_id);
      };
        ...
        return (
            <React.Fragment>
                {!this.state.loading &&
                    <React.Fragment>
                        <Dashboard
                            title={this.state.dict_mapped_data.length ? <span className="table-title-1"> Total cases awaiting invite acceptance: {this.state.dict_mapped_data.length || 0}</span> : <React.Fragment />}
                            ...
                            noDataText={'No cases awaiting invite acceptance'}
                            dateFormat='MMMM dd, yyyy'
                            filterDateLabel={'Filter by Invited Date'}
                        />
                        {/* <h5 className="no-data-text">No cases awaiting invite acceptance</h5> */}
                    </React.Fragment>
                }
...
export default withRouter(withIonLifeCycle(PendingInvites));
