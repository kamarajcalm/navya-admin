...
class RejectedOrIgnoredCases extends React.PureComponent {
    ...
    customDateFilter = async (startDate, endDate) => {
        await this.getRejectedOrIgnoredCases(formatDate(startDate), formatDate(endDate));
    };
    ...
    getRejectedOrIgnoredCases = async (startDate, endDate) => {
        ...
        const response = await api.getRejectedOrIgnoredCases(payload);
        if (response && response.success && response.data && response.data.result) {
            let finalData = response.data.result.map(v => {
                return {
                    ...
                    date_rejected: v['date_rejected']? new Date(v['date_rejected']).toLocaleDateString('en-us', { year: "numeric", month: "short", day: "numeric", hour12: true, hour: '2-digit', minute: '2-digit' }) : "",
                    gender: v['gender'],
                    state: v["state"],
                    fp: v["fp"],
                    ...
                    next_assignee: v['next_assignee']
                }
            })
            const newColumns = response.data.columns.map(v => {
                if (v.field === 'date_rejected' || v.field === 'date_assigned') {
    ...
    viewClick = (row) => {
        this.props.history.push("/user/dashboard/rejected-or-ignored/" + row.survey_id);
      };
        ...
        return (
            <React.Fragment>
                {!this.state.loading &&
                    <React.Fragment>
                        <Dashboard
                            title={this.state.dict_mapped_data.length ? <span className="table-title-1">Total cases rejected or ignored: {this.state.dict_mapped_data.length || 0}</span> : <React.Fragment />}
                            ...
                            noDataText={'No cases rejected or ignored'}
                            dateFormat='MMMM dd, yyyy'
                            filterDateLabel={'Filter by Rejected Date'}
                        />
                        {/* <h5 className="no-data-text">No cases rejected or ignored</h5> */}
                    </React.Fragment>
                }
...
export default withRouter(withIonLifeCycle(RejectedOrIgnoredCases));
