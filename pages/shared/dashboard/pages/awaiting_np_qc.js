...
class AwaitingNPQC extends React.PureComponent {
    ...
    customDateFilter = async (startDate, endDate) => {
        await this.getNPQCPendingCaseStatus(formatDate(startDate), formatDate(endDate));
    };

    getNPQCPendingCaseStatus = async (startDate, endDate) => {
        ...
        const response = await api.getNPQCPendingCaseStatus(payload);
        if (response && response.success && response.data && response.data.result) {
            ...
            const newColumns = response.data.columns.map(v => {
                if (v.field === 'case_due_date' || v.field === 'date_assigned' || v.field === 'expert_completed_date') {
    ...
    viewClick = (row) => {
        this.props.history.push("/user/dashboard/npqccompleted/" + row.survey_id);
    };
        ...
        return (
            <React.Fragment>
                {!this.state.loading &&
                    <React.Fragment>
                        <Dashboard
                            title={this.state.dict_mapped_data.length ? <span className="table-title-1"> Awaiting NP QC cases: {this.state.dict_mapped_data.length || 0} </span> : <React.Fragment />}
                            ...
                            noDataText={'No NP QC Awaiting cases'}
...
export default withRouter(withIonLifeCycle(AwaitingNPQC));
