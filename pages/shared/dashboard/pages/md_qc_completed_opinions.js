...
class MDQCCompletedOpinions extends React.PureComponent {
    ...
    customDateFilter = async (startDate, endDate) => {
        await this.getMdQcCompletedOpinions(formatDate(startDate), formatDate(endDate));
    };

    getMdQcCompletedOpinions = async (startDate, endDate) => {
        ...
        const response = await api.getMdQcCompletedOpinions(payload);
    ...
    viewClick = (row) => {
        this.props.history.push("/user/dashboard/md-qc-complete/" + row.survey_id);
      };
        ...
        return (
            <React.Fragment>
                {!this.state.loading &&
                    <React.Fragment>
                        <Dashboard
                            title={this.state.dict_mapped_data.length ? <span className="table-title-1"> Total MD QC completed cases: {this.state.dict_mapped_data.length || 0} </span> : <React.Fragment />}
                            ...
                            noDataText={'No MD QC completed cases'}
...
export default withRouter(withIonLifeCycle(MDQCCompletedOpinions));