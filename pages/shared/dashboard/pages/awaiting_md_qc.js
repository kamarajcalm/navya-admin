...
class AwaitingMDQC extends React.PureComponent {

    constructor(props) {
        super(props);
        this.state = {
            dict_mapped_data: [], columns: [], loading: false, viewButton: {
                title: "",
                field: "",
                align: "center",
                render: (rowData) => (
    ...
    customDateFilter = async (startDate, endDate) => {
        await this.getMDQCPendingCaseStatus(formatDate(startDate), formatDate(endDate));
    };

    getMDQCPendingCaseStatus = async (startDate, endDate) => {
        ...
        const payload = { 'start_date': new Date(startDate).toISOString(), 'end_date': new Date(updatedEndDate).toISOString() };
        const response = await api.getMDQCPendingCaseStatus(payload);
        if (response && response.success && response.data && response.data.result) {
            ...
            response.data.result.forEach(v => {
                finalData.push({
                    ...
                    expert_completed_date: v['expert_completed_date'] ? new Date(v['expert_completed_date']).toLocaleDateString('en-us', { year: "numeric", month: "short", day: "numeric", hour12: true, hour: '2-digit', minute: '2-digit' }) : "",
                    np_qc_completed_date: v['np_qc_completed_date'] ? new Date(v['np_qc_completed_date']).toLocaleDateString('en-us', { year: "numeric", month: "short", day: "numeric", hour12: true, hour: '2-digit', minute: '2-digit' }) : "",

                })
            })
            const newColumns = response.data.columns.map(v => {
                if (v.field === 'case_due_date' || v.field === 'date_assigned' || v.field === 'expert_completed_date' || v.field === 'np_qc_completed_date') {
    ...
    viewClick = (row) => {
        this.props.history.push("/user/dashboard/mdqccompleted/" + row.survey_id);
    };
        ...
        return (
            <React.Fragment>
                {!this.state.loading &&
                    <React.Fragment>
                        <Dashboard
                            title={this.state.dict_mapped_data.length ? <span className="table-title-1"> Awaiting MD QC cases: {this.state.dict_mapped_data.length || 0} </span> : <React.Fragment />}
                            ...
                            noDataText={'No MD QC Awaiting cases'}
                            dateFormat='MMMM dd, yyyy'
                            filterDateLabel={'Filter by Assigned Date'}
                        />
                    </React.Fragment>
                }
...
export default withRouter(withIonLifeCycle(AwaitingMDQC));