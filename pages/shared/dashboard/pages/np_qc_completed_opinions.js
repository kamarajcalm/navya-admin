...
class NPQCCompletedOpinions extends React.PureComponent {
    ...
    customDateFilter = async (startDate, endDate) => {
        await this.getNPQcCompletedOpinions(formatDate(startDate), formatDate(endDate));
    };

    getNPQcCompletedOpinions = async (startDate, endDate) => {
        ...
        const response = await api.getNPQcCompletedOpinions(payload);
        if (response && response.success && response.data && response.data.result) {
            ...
            const newColumns = response.data.columns.map(v => {
                if (v.field === 'case_due_date' || v.field === 'date_assigned' || v.field === 'date_completed') {
    ...
    viewClick = (row) => {
        this.props.history.push("/user/dashboard/np-qc-complete/" + row.survey_id);
      };
        ...
        return (
            <React.Fragment>
                {!this.state.loading &&
                    <React.Fragment>
                        <Dashboard
                            title={this.state.dict_mapped_data.length ? <span className="table-title-1"> Total NP QC completed cases: {this.state.dict_mapped_data.length || 0} </span> : <React.Fragment />}
                            ...
                            noDataText={'No NP QC completed cases'}
...
export default withRouter(withIonLifeCycle(NPQCCompletedOpinions));
