...
import { isNavya, isMSKCC, isPM, isAccessHope } from '../../../referrers';
...
class LoginComponent extends React.PureComponent {
  ...
  setPermissions = async () => {
    try {
      ...
      if (permissions?.data?.length) {
        tokenManager.setPermissions(JSON.stringify(permissions.data));
        // window.localStorage.setItem('user_permissions', JSON.stringify(permissions.data));  
      }
    }
    catch (err) {
      console.log(err);
  ...
  onFormSubmit = async (values={}, hideLoader, setShowHideAlert) => {
    ...
    this.setState({
      isOpen: false,
      redirect: true,
      redirectURL: isAlphaPreviewParamsInURL() ? `/user/alphasurveypreview/${alphamedicalcaseid}/${alphaeosid}`: surveyID ? `/user/surveys/${surveyID}` : hasOnlyDashboardAccess() ? '/user/dashboard' : '/user/home'
                ...
                <div className="login-outer-container">
                  ...
                  <div className="login-container">
                      ...
                      <Form
                        ...
                        errorMessage={(isMSKCC() ? 'MSKCC email' : isPM() ? 'UHN/PM email' : isAccessHope() ? 'AccessHope email' : 'Username') + ' and Password is required'}
                        buttons={[
                          {
                            text: 'OK',
                            role: 'cancel',
